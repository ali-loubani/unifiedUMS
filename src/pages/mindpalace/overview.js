...
import { OverviewTotalProfit } from "../../sections/mindpalace/overview/overview-total-profit";
import { OverviewTotalSubscribers } from "../../sections/mindpalace/overview/overview-total-subscribers";
import { OverviewTotalUnsubscribedSubscribers } from "../../sections/mindpalace/overview/overview-total-unsubscribed-subscribers";
import { OverviewTotalRenewal } from "../../sections/mindpalace/overview/overview-total-renewal";
import { useState, useEffect } from "react";
import { OverviewSubType } from "../../sections/mindpalace/overview/overview-sub-type";
import Cookies from "js-cookie";
import { OverviewTotalDeactives } from "../../sections/mindpalace/overview/overview-total-deactives";
import { OverviewTotalRegisters } from "../../sections/mindpalace/overview/overview-total-registers";
import { OverviewTotalUnsuccessful } from "../../sections/mindpalace/overview/overview-total-unsuccessful";
import { OverviewTotalUnknowns } from "../../sections/mindpalace/overview/overview-total-unknowns";
import { OverviewMonthGrowth } from "../../sections/mindpalace/overview/overview-month-growth";
import { BeatLoader, MoonLoader } from "react-spinners";
import { DateTime } from "luxon";
import { OverviewSales } from "../../sections/mindpalace/overview/overview-sales";
import { ShortCode } from "../../sections/mindpalace/overview/overview-shortcode";
import { OverviewTotalMobivert } from "../../sections/mindpalace/overview/overview-mobivert";
...
const Page = () => {
  ...
  useEffect(() => {
    checkToken();
    fetchData(Cookies.get("mindtoken"));
    ...
    try {
      ...
      if (operator == "0") {
        response = await fetch(
          "http://89.232.186.173:8088/api/statistics?from=" + dateFrom + "&to=" + dateTo,
          {
            headers: {
              "Content-Type": "application/json",
              accept: "application/json",
        ...
        response = await fetch(
          "http://89.232.186.173:8088/api/statistics?from=" +
            dateFrom +
            "&to=" +
            dateTo +
            "&operator=" +
  ...
  const fetchData = async (token) => {
    try {
      const response = await fetch("http://89.232.186.173:8088/api/statistics", {
        headers: {
          "Content-Type": "application/json",
          accept: "application/json",
          Authorization: "Bearer " + token,
  ...
  //     const url = `http://89.232.186.173:8088/api/statistics?from=${year}-${month}-01&to=${year}-${month}-31`;
  ...
  const handlefilter = () => {
    fetchDataDate(Cookies.get("mindtoken"));
  };
  ...
  return (
    <>
      {loadingPage ? (
        ...
        <>
          <Head>
            <title>Overview | Mind Palace</title>
          </Head>
          <Box
            component="main"
            sx={{
                ...
                >
                  ...
                  <Grid item 
                    ...
                    <OverviewMonthGrowth // CALLING OVERVIEW-TOTAL-UNSUBS FILE
                      sx={{ height: "100%" }}
                      loading={loading}
                      Data={data.Month_over_month_growth}
                    />
                  </Grid>
                <Grid item 
                  xs={12}>
