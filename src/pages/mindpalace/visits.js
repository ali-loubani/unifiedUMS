...
import { VisitsSearch } from "../../sections/mindpalace/visits/visits-search";
import { VisitsTable } from "../../sections/mindpalace/visits/visits-table";
...
const Page = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch("http://89.232.186.173:8088/api/visits", {
        headers: {
          "Content-Type": "application/json",
          accept: "application/json",
          Authorization: "Bearer " + Cookies.get("mindtoken"),
  ...
  const fetchDataDate = async () => {
    ...
    try {
      const response = await fetch(
        "http://89.232.186.173:8088/api/visits?from=" + dateFrom + "&to=" + dateTo,
        {
          headers: {
            "Content-Type": "application/json",
            accept: "application/json",
  ...
  const handleExport = () => {
    ...
    const worksheet = XLSX.utils.table_to_sheet(document.getElementById("visits"));
  ...
  return (
    <>
      {loadingPage ? (
        ...
        <>
          <Head>
            <title>Visits | Mind Palace</title>
          </Head>
          <Box
            component="main"
            sx={{
          ...
          >
            <Container maxWidth="xl">
              <Stack direction="row" 
                ...
                spacing={4}>
                <Stack spacing={1}>
                  <Typography variant="h4">Total Visits</Typography>
                </Stack>
              </Stack>
              <Grid container 
                alignItems="center" 
              ...
              <VisitsSearch onSearch={handleSearch} />
              <VisitsTable
                data={data}
                loading={loading}
                dateFrom={dateFrom}
