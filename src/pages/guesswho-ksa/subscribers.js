...
import { Button, Grid, Input, SvgIcon, TextField, Typography } from "@mui/material";
import { Box, Container, Stack } from "@mui/system";
import Head from "next/head";
import { useEffect, useState } from "react";
import { Layout as DashboardLayout } from "src/layouts/dashboard/layout";
import { SubscriberStat } from "../../sections/guesswho-ksa/subscribers/subscribers-stat";
import { SubscribersSearch } from "../../sections/guesswho-ksa/subscribers/subscriber-search";
import { SubscribersTable } from "../../sections/guesswho-ksa/subscribers/subscriber-table";
...
const Page = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        "http://guesswhoservice.com/api/subscribers.php"
        );
      const jsonData = await response.json();
      setData(jsonData);
    } catch (error) {
  ...
  const fetchDataDate = async () => {
    ...
    try {
      const response = await fetch(
        "http://guesswhoservice.com/api/subscribers.php?from=" + dateFrom + "&to=" + dateTo
      );
      const jsonData = await response.json();
      setData(jsonData);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    data.forEach((subscriber) => {
      if (subscriber.operatorId == 1) {
        ...
        updatedOperator.Mobily += 1;
      } else if (subscriber.operatorId == 2) {
        // UMS SUBSCRIBRES
        updatedOperator.Zain += 1;
      } else if (subscriber.operatorId == 3) {
        updatedOperator.Stc += 1 ;
      }
    });
  ...
  const Blocklist = async () => {
    ...
    try {
      ...
      const response = await fetch("http://guesswhoservice.com:8084/api/v1/blockList", {
        method: "POST",
        headers: {
          accept: "application/json",
        },
  ...
  const Unsublist = async () => {
    ...
    try {
      ...
      const response = await fetch("http://guesswhoservice.com:8084/api/v1/unSubList", {
        method: "POST",
        headers: {
          accept: "application/json",
        },
