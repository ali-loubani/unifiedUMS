...
import { SubscribersTable } from "src/sections/gamebay/subscribers/subscribers-table";
import { SubscribersSearch } from "src/sections/gamebay/subscribers/subscribers-search";
import { useState, useEffect } from "react";
import { SubscriberStat } from "../../sections/gamebay/subscribers/subscriber-stat";
...
const Page = () => {
  ...
  useEffect(() => {
    checkToken();
  }, [searchQuery, operator, dateFrom, dateTo]);
  ...
  const fetchDataFirst = async () => {
    setLoading(true);
    try {
      const response = await fetch("http://82.148.2.56:8088/api/subscribers", {
        headers: {
          "Content-Type": "application/json",
          accept: "application/json",
          Authorization: "Bearer " + Cookies.get("usertoken"),
  ...
  const fetchData = async (page) => {
    setLoading(true);
    try {
      const response = await fetch(`http://82.148.2.56:8088/api/subscribers?page=${page}`, {
        headers: {
          "Content-Type": "application/json",
          accept: "application/json",
          Authorization: "Bearer " + Cookies.get("usertoken"),
  ...
  useEffect(() => {
    ...
    filteredData.forEach((subscriber) => {
      ...
      } else if (subscriber.operator_id === 2) {
        // UCELL SUBSCRIBRES
        updatedOperator.Ucell += 1;
      }
    });
  ...
  const Blocklist = async () => {
      ...
      const response = await fetch("http://82.148.2.56:8088/api/blockList", {
        method: "POST",
        headers: {
          accept: "application/json",
          Authorization: "Bearer " + Cookies.get("usertoken"),
  ...
  const Unsublist = async () => {
    ...
    try {
      ...
      const response = await fetch("http://82.148.2.56:8088/api/unsubList", {
        method: "POST",
        headers: {
          accept: "application/json",
          Authorization: "Bearer " + Cookies.get("usertoken"),
  ...
  return (
    ...
    >
      ...
      <>
      <Head>
        <title>Subscribers | Game Bay</title>
      </Head>
