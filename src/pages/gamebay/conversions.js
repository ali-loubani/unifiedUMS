...
import { ConversionsSearch } from "../../sections/gamebay/conversions/conversion-search";
import { ConversionsTable } from "../../sections/gamebay/conversions/conversion-table";
...
const Page = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch("http://82.148.2.56:8088/api/conversions", {
        headers: {
          "Content-Type": "application/json",
          accept: "application/json",
          Authorization: "Bearer " + Cookies.get("usertoken"),
  ...
  const fetchDataDate = async () => {
    ...
    try {
      const response = await fetch(
        "http://82.148.6.228:8088/api/conversions?from=" +
          dateFrom +
          "&to=" +
          dateTo,
        {
  ...
  return (
    <>
      {loadingPage ? (
        ...
        <>
          <Head>
            <title>Conversions | GameBay</title>
          </Head>
          <Box
            component="main"
            sx={{
