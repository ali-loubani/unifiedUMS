...
import { DailyTable } from "../../sections/gamebay/daily-statistics/daily-table";
...
const Page = () => {
  ...
  const fetchDataDate = async () => {
    ...
    try {
      const response = await fetch(
        "http://82.148.2.56:8088/api/dailyStatistics?from=" +
          dateFrom +
          "&to=" +
          dateTo +
          "&operator=" +
  ...
  const handleExport = () => {
    ...
    const worksheet = XLSX.utils.table_to_sheet(
      document.getElementById("daily-gamebay")
    );
  ...
  return (
    <>
      {loadingPage ? (
        ...
        <>
          <Head>
            <title>Daily Statistics | Game Bay</title>
          </Head>
          <Box
            component="main"
            sx={{
