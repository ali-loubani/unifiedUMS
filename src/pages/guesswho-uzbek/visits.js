...
import { VisitsSearch } from "../../sections/guesswho-uzbek/visits/visits-search";
import { VisitsTable } from "../../sections/guesswho-uzbek/visits/visits-table";
...
const Page = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch("http://82.148.6.228:3000/visits");
  ...
  const fetchDataDate = async () => {
    ...
    try {
      const response = await fetch(
        "http://82.148.6.228:3000/visits?from=" + dateFrom + "&to=" + dateTo
      );
      const jsonData = await response.json();
      setData(jsonData.results);
    } catch (error) {
