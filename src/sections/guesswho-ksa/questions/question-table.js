...
export const QuestionsTable = ({ searchQuery = '' }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch("http://guesswhoservice.com/api/questions.php");
...
const filteredData = data && data.filter((question) => {
    return (
      ...
      question.arabic.includes(searchQuery)
    );
  });
                      ...
                      .map((question) => {
                        return (
                          ...
                          key={question.id}>
                            ...
                            <TableCell align="center">{question.arabic}</TableCell>
                            <TableCell align="center">
                              <img width={50}
                                height={50}
                                style={{cursor: 'pointer'}}
                                src={'http://guesswhoservice.com/cms/media/' + question.media}
                                onClick={() => {
                                  window.open('http://guesswhoservice.com/cms/media/' + question.media)
                                }} />
                              </TableCell>
