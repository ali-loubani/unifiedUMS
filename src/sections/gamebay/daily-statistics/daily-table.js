...
export const DailyTable = ({ data, loading, dateFrom, dateTo }) => {
  ...
  const totalSubs = filteredData
    ? filteredData.reduce(
        (total, daily) => total + parseInt(daily.totalSubs?.SubCount ?? 0),
        0
      )
    : 0;
  ...
  const totalCharged = filteredData
    ? filteredData.reduce(
        (total, daily) => total + parseInt(daily.charged?.charged ?? 0),
        0
      )
    : 0;
  ...
  const totalCharges = filteredData
    ? filteredData.reduce(
        (total, daily) => total + parseInt(daily.charging?.chargeAmount ?? 0),
        0
      )
    : 0;
                        ...
                        return (
                          <TableRow hover key={"dd-" + index}>
                            ...
                            <TableCell align="center">
                              {daily.totalSubs?.Date ??
                                daily.charged.Date ??
                                daily.charging.Date}
                            </TableCell>
                            <TableCell align="center">
                              {daily.totalSubs?.SubCount ?? 0}
                            </TableCell>
                            <TableCell align="center">
                              {daily.activeSubs ?? 0}
                            </TableCell>
                            <TableCell align="center">
                              {daily.charged?.charged ?? 0}
                            </TableCell>
                            <TableCell align="center">
