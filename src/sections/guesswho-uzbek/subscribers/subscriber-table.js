...
export const SubscribersTable = (props) => {
  ...
  const filteredData = data?.filter((subscriber) => {
    return (
      subscriber.mobile.toString().includes(searchQuery) &&
  ...
  const handleConfirmUnsub = async (mobile) => {
    try {
      const response = await fetch(
        "http://82.148.6.228:8084/api/v1/unsubscribe",
        {
          method: "POST",
          headers: {
  ...
  return (
    <Card>
                    ...
                    return (
                      <TableRow hover 
                        ...
                        <TableCell align="center">
                          {subscriber.registerDate}
                        </TableCell>
                        <TableCell align="center">
                          {subscriber.mobile}
                        </TableCell>
                        <TableCell align="center">
                          {subscriber.mobileVerif}
                        </TableCell>
                        <TableCell align="center">
                        ...
                        <TableCell align="center">
                          {subscriber.campaignId == 4
                            ? "S1"
                            : subscriber.campaignId == 10
                            ? "Hendrick"
                        ...
                        <TableCell align="center">
                          {subscriber.deactiveDate}
                        </TableCell>
                        <TableCell align="center">
                          {subscriber.blocked == 1 ? "yes" : "no"}
                        </TableCell>
                        <TableCell align="center">
                          {subscriber.questionAllowed}
                        </TableCell>
                        <TableCell align="center">
                          {subscriber.totalPoints}
                        </TableCell>
                        <TableCell align="center">
                          {subscriber.totalCharge}
                        </TableCell>
                        <TableCell align="center">
                          {subscriber.sendRequestDate}
                        </TableCell>
      ...
      <TablePagination
        ...
        component="div"
        count={filteredData ? Number(filteredData?.length) : 0}
