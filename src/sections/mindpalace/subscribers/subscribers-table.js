...
export const SubscribersTable = ( props ) => {
  ...
  useEffect(() => {
    setFilteredData(
      ...
      data
      ?.filter((subscriber) => {
          const mobileMatch = subscriber.mobile.toString().includes(searchQuery);
  ...
  const handleConfirmUnblock = async (id) => {
    ...
    try {
      const response = await fetch("http://89.232.186.173:8088/api/unblock/" + id, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          accept: "application/json",
  ...
  const handleConfirmUnsub = async (mobile) => {
    ...
    try {
      const response = await fetch("http://89.232.186.173:8088/api/unsub", {
        ...
        body: JSON.stringify({ mobile: mobile, lang: "en-US", channel: "SYSTEM" }),
      });
  ...
  return (
    <Card>
                ...
                .map((subscriber, index) => {
                  ...
                  return (
                    <TableRow hover
                      ...
                      <TableCell align="center">{subscriber.game_iq}</TableCell>
                      <TableCell align="center">{subscriber.played_games}</TableCell>
                      <TableCell align="center">{subscriber.allowed_games}</TableCell>
                      <TableCell align="center">{subscriber.collected_points}</TableCell>
                      <TableCell align="center">
                        <img width={50}
                          height={50}
                          src={'/' + subscriber.profile_photo}
                          style={{borderRadius:'50%'}}/>
                      </TableCell>
                      <TableCell align="center">
                        {subscriber.is_blocked === 1 ? "yes" : "no"} {/* IF BLOCKED => YES ELSE NO */}
                      </TableCell>
                      <TableCell align="center">{subscriber.created_at.substring(0,10)}</TableCell>
                      <TableCell>
                        <div style={{ display: "flex", justifyContent: "center", width: "100%" }}>
                          {subscriber.is_blocked == "0" ? (
                            subscriber.is_subscribed || subscriber.is_subscription_active ? (
                              <>
                                <Button
                                  variant="contained"
                                  color="primary"
      ...
      <TablePagination
        rowsPerPageOptions={[50, 100, 250,  Number((filteredData && filteredData?.length) > 50 && filteredData.length)]}
        component="div"
